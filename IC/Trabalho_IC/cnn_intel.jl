...
NCLASSES = 0
USING_MODEL = "CNN_INTEL_NOVO3.bson"
...
function teste2_model()
    classes = readdir(dir_imgs_pre_process_train)
    BSON.@load USING_MODEL modelo
    numero_certo = 0
    numero_imagem = 0
    println("Treinando com toda base de testes:")
    for classe in classes
        nome_imgs = readdir(string(dir_imgs_pre_process_test, classe))
        for nome_img in nome_imgs
            # configura a imagem para classificar como e configurado no modelo
            img = load(string(dir_imgs_pre_process_test, classe,"/",  nome_img))
            img = channelview(img)
            img = reshape(img, (1,3,28,28))
            img = permutedims(img, (4,3,2,1))
            img = convert(Array{Float32,4}, img)
    ...
    acuracia = numero_certo / numero_imagem
    
    @info(string("Total de imagens: ", numero_imagem))
    @info(string("Total de imagens predizidas corretamente: ", numero_certo))
    @info(string("Acuracia: ", acuracia))
end

function main()
    println("Trabalho de Arthur e Vinnicius")
    println("Selecione 1 para treinar a rede | Selecione 2 para testar rede treinada | Selecione 3 para testar completo")
